...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    for sub_dir in sub_dirs:
        ...
        label_name_number = float(label_name[4:])
        training_images = []
        testing_images = []
        validation_images = []
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
    ...
    if how_many >= 0:
        ...
        for unused_i in range(how_many):
            ...
            ground_truth_number = image_lists[label_name]['year_number']
    else:
        ...
        for label_index, label_name in enumerate(image_lists.keys()):
            for image_index, image_name in enumerate(
                ...
                filenames.append(image_name)
                ground_truth_number = image_lists[label_name]['year_number']
    return bottlenecks, ground_truths, ground_truth_number, filenames
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
    ...
    with tf.name_scope('input'):
        ...
        ground_truth_number = tf.placeholder(tf.float32,
                                             name='GroundTruthNumber')
    ...
    return (logits, bottleneck_input, ground_truth_input, ground_truth_number,
            final_tensor)

def add_loss_and_train(logits, ground_truth_input, ground_truth_number):
    with tf.name_scope('cross_entropy'):
        cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
            labels=ground_truth_input, logits=logits)

        predicted_class = tf.placeholder(tf.float32,
                                        name='predicted_class')
        with tf.name_scope('mse'):
            mse = tf.losses.mean_squared_error(predicted_class, ground_truth_number)
            tf.summary.scalar('mse', mse)
        with tf.name_scope('total'):
            total_loss = (1-FLAGS.mse_ratio) * tf.reduce_mean(cross_entropy) + FLAGS.mse_ratio * mse
    ...
    return train_step, total_loss, predicted_class
...
def main(_):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        (logits, bottleneck_input, ground_truth_input, ground_truth_number,
         final_tensor) = add_final_training_ops(len(image_lists.keys()),
                                                FLAGS.final_tensor_name,
                                                bottleneck_tensor)

        train_step, total_loss, predicted_class = add_loss_and_train(logits,
                                    ground_truth_input, ground_truth_number)
        ...
        for i in range(FLAGS.how_many_training_steps):
            ...
            if do_distort_images:
                ...
                (train_bottlenecks,
                 train_ground_truth, train_ground_truth_number, _) = get_random_cached_bottlenecks(
                    sess, image_lists, FLAGS.train_batch_size, 'training',
            ...
            train_summary, _ = sess.run(
                ...
                feed_dict={bottleneck_input: train_bottlenecks,
                           ground_truth_input: train_ground_truth,
                           ground_truth_number: train_ground_truth_number,
            ...
            if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
                train_accuracy, total_loss_value = sess.run(
                    ...
                    feed_dict={bottleneck_input: train_bottlenecks,
                               ...
                               ground_truth_number: train_ground_truth_number,
                               predicted_class: train_class,})
                validation_bottlenecks, validation_ground_truth, validation_ground_truth_number, _ = (
                ...
                validation_summary, validation_accuracy = sess.run(
                    ...
                    feed_dict={bottleneck_input: validation_bottlenecks,
                               ground_truth_input: validation_ground_truth,
                               ground_truth_number: validation_ground_truth_number,
        ...
        test_bottlenecks, test_ground_truth, test_ground_truth_number, test_filenames = (
        ...
        test_accuracy, predictions = sess.run(
            ...
            feed_dict={bottleneck_input: test_bottlenecks,
                       ground_truth_input: test_ground_truth,
                       ground_truth_number: test_ground_truth_number,
